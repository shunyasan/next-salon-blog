...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const paths = [...Array(num)].map((_, i) => `/clinic/chuou/${i + 1}`);
...
const ClinicsChuou: NextPage<Props> = ({ area, clinics, page }) => {
  ...
  return (
    <>
      <Head>
        <title>中央区の医療脱毛クリニック一覧 | 脱毛コンサルタント</title>
        <meta
          name="description"
      ...
      <ClinicListTemplate
        title="中央区の医療脱毛クリニック一覧"
        areaId={areaId}
        areaMax={numOfClinicMax}
        area={area || []}
        clinics={clinics}
        page={page}
        getPage={(page) => router.push(`/clinic/chuou/${page + 1}`)}
      />
    </>
  );
};
export default ClinicsChuou;
