...
export const getStaticPaths: GetStaticPaths = async () => {
  ...
  const paths = arr.map((ge) => `/${ge}/treatment-parts/other`);
...
const TreatmentOtherParts: NextPage<Props> = ({ about, gender }) => {
  ...
  return (
    ...
    >
      <Head>
        <title>その他の脱毛可能な部位 | 脱毛コンサルタント</title>
        <meta
          name="description"
          content="「渋谷・恵比寿・新宿・銀座・六本木・池袋」大手から優良小規模で脱毛可能な部位です。おすすめの部位から意外な部位まで掲載しています。"
        />
      </Head>
      <BgImgH1 title="その他の脱毛可能な部位" />
    </TreatmentTemplete>
  );
};
export default TreatmentOtherParts;
